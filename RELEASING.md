...
# Releasing pain001-loader-mt101
...
core: when `pain001` ships `0.0.X`, pain001-loader-mt101 ships `0.0.X`, even when nothing
...
The suite releases together, so a release of pain001-loader-mt101 is usually the suite
...
4. Release notes exist at `releases/v0.0.X.md`, or the changelog section stands in for them; the release workflow fails without either.
5. The version is identical in `pyproject.toml` and `pain001_loader_mt101/__init__.py`.

## Cutting the release

1. Bump the version and add the changelog section in one PR, titled
   `chore(release): align pain001-loader-mt101 on 0.0.X`.
2. Merge it once CI is green.
3. Push a signed tag:

   ```bash
   git tag -s v0.0.X -m "pain001-loader-mt101 v0.0.X" <merge-commit>
...
- Confirm the version on [PyPI](https://pypi.org/project/pain001-loader-mt101/) and that
  the GitHub release is published, with the SBOMs attached.
- Verify a clean install: `pip install pain001-loader-mt101==0.0.X`.
